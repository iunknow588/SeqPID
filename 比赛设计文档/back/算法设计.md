...
> 本文统一采用“成交量窗口即交易时间 `q`”的理论口径：价格状态写作 `P(q)`，`Delta_q P` 是速度，`Delta_q^2 P` 是加速度；二阶公式是理论主公式，一阶公式若出现仅作工程近似。
...
这里补充一个关键口径：算法实现中所有窗口级差分默认都是相邻交易时间步 `q` 的差分，而不是在理论层面把自然时钟秒数直接代入状态方程。这样规则层、PID 层和后续稳定性分析才共享同一坐标系。
